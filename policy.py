...
class Policy(nn.Module):
    def __init__(self, input_dim, hidden_dim ,dropout_rate, lstm_num_layers, word_emb_dim, node_emb_dim):
    ...
    def init_path(self, init_action):
        ...
        init_action = torch.Tensor(init_action).view(self.batch_size, 1, -1)
    ...
    def update_path(self, action):
        action = torch.Tensor(action).view(self.batch_size, 1, -1)
        self.path.append(self.lstm_cell(action, self.path[-1])[1])
